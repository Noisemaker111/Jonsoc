...
1. **Hosted models require specific infrastructure** - Models like minimax/m2.1 are hosted by jonsoc.com and require their proxy services
...
- ✅ Free models infrastructure (jonsoc.com's models.dev)
...
This makes the intent clear: you're choosing to not use jonsoc.com's models infrastructure.

## Problem

The original codebase had over 100 hardcoded references to "jonsoc.com" and "jonsoc.com" scattered across multiple files. This made forking difficult as developers had to:
...
Created a centralized `Brand` namespace (`packages/jonsoc/src/brand/index.ts`) that serves as a single source of truth for all brand-specific configuration.

## New File

### `packages/jonsoc/src/brand/index.ts` (NEW)
...
export namespace Brand {
  ...
  export const DOMAIN = "jonsoc.com"
  export const API_URL = "https://api.jonsoc.com"
  export const MODELS_URL = "https://models.dev"  // Defaults to jonsoc.com's models.dev
  export const INSTALL_URL = "https://jonsoc.com/install"
  export const CONFIG_SCHEMA_URL = "https://jonsoc.com/config.json"
...
1. **`packages/jonsoc/src/global/index.ts`**
   - Uses `Brand.MODELS_URL` for models API
   - Uses `Brand.APP_NAME` for XDG paths

2. **`packages/jonsoc/src/config/config.ts`**
...
3. **`packages/jonsoc/src/installation/index.ts`**
...
4. **`packages/jonsoc/src/provider/provider.ts`**
...
5. **`packages/jonsoc/src/cli/cmd/github.ts`**
...
For permanent changes, edit `packages/jonsoc/src/brand/index.ts` and modify the default values. This is recommended for production forks.
...
2. **Legacy directory names**: Still checks for `.jonsoc/` and `jonsoc.json` files when brand is "jonsoc"
3. **Legacy provider support**: Original "jonsoc" provider still works
...
- **Clear separation: branding = yours, models = jonsoc.com's (optional override)**
...
| `JONSOC_CONFIG_SCHEMA` | JSON Schema URL          | `https://jonsoc.com/config.json`      |
...
**Note on Models**: `OPENCODE_MODELS_URL` is intentionally NOT configured by domain. This makes it clear when someone wants to use custom models infrastructure. The default uses jonsoc.com's models.dev, providing access to hosted models like minimax/m2.1.
...
### Scenario 1: Rebrand with jonsoc.com Models (Most Common)
...
# - Still get minimax/m2.1 and other jonsoc.com hosted models
...
# - No access to jonsoc.com hosted models unless you add them manually
```

---
title: Providers
description: Using any LLM provider in JonsOC.
---

import config from "../../../config.mjs"
export const console = config.console

JonsOC uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your JonsOC config.
...
in `~/.local/share/jonsoc/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your JonsOC
config.
...
```json title="jonsoc.json" {6}
{
  "$schema": "https://jonsoc.ai/config.json",
  "provider": {
    "anthropic": {
      "options": {
...
JonsOC Zen is a list of models provided by the JonsOC team that have been
tested and verified to work well with JonsOC. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with JonsOC Zen.
:::

1. Run the `/connect` command in the TUI, select jonsoc, and head to [jonsoc.ai/auth](https://jonsoc.ai/auth).
...
It works like any other provider in JonsOC and is completely optional to use.
...
To use Amazon Bedrock with JonsOC:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running jonsoc:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY jonsoc

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile jonsoc

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX jonsoc
   ```
   ...
   For project-specific or persistent configuration, use `jonsoc.json`:

   ```json title="jonsoc.json"
   {
     "$schema": "https://jonsoc.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
:::info
We've received reports of some users having their subscriptions blocked while using it with JonsOC.
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for jonsoc to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX jonsoc
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX jonsoc
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your jonsoc config.
...
to store token in jonsoc auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
JonsOC uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock JonsOC
to only use your own GitLab-hosted instance, add the following to your
`jonsoc.json` file. It is also recommended to disable session sharing.
...
More documentation on [jonsoc-gitlab-auth](https://www.npmjs.com/package/@gitlab/jonsoc-gitlab-auth) homepage.

##### Configuration

Customize through `jonsoc.json`:
...
{
  "$schema": "https://jonsoc.ai/config.json",
  "plugin": ["@gitlab/jonsoc-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with jonsoc:
...
To use Google Vertex AI with JonsOC:
...
2. Set the required environment variables:
   ...
   Set them while running jonsoc.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id jonsoc
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through jonsoc, you can always configure it yourself.
...
```jsonc title="~/.config/jonsoc/jonsoc.jsonc"
{
  "$schema": "https://jonsoc.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [jonsoc-helicone-session](https://github.com/H2Shami/jonsoc-helicone-session) plugin to automatically log each JonsOC conversation as a session in Helicone.

```bash
npm install -g jonsoc-helicone-session
```
...
```json title="jonsoc.json"
{
  "plugin": ["jonsoc-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each JonsOC conversation listed as a separate session.
...
You can configure jonsoc to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="jonsoc.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://jonsoc.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure jonsoc to use local models through LM Studio.

```json title="jonsoc.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://jonsoc.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure jonsoc to use local models through Ollama.

```json title="jonsoc.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://jonsoc.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with JonsOC:
...
3. Copy the API key for use in JonsOC.
...
6. **Important**: Before using cloud models in JonsOC, you must pull the model information locally:
...
JonsOC Zen is a list of tested and verified models provided by the JonsOC team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>JonsOC Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **JonsOC Zen**.

   ```txt
   /connect
   ```

3. Enter your JonsOC API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your jonsoc config.
...
5. You can also customize them through your jonsoc config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' jonsoc
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group jonsoc
   ```
...
You can also customize models through your jonsoc config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with jonsoc. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in jonsoc.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `jonsoc.json` file in your project directory:

   ```json title="jonsoc.json" ""myprovider"" {5-15}
   {
     "$schema": "https://jonsoc.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="jonsoc.json" {9,11,17-20}
{
  "$schema": "https://jonsoc.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow JonsOC to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `jonsoc auth list` to see if the credentials
...
2. For custom providers, check the jonsoc config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your jonsoc config.

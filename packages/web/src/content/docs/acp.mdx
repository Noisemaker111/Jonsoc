---
title: ACP Support
description: Use JonsOC in any ACP-compatible editor.
---

JonsOC supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use JonsOC via ACP, configure your editor to run the `jonsoc acp` command.

The command starts JonsOC as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
```json title="acp.json"
{
  "agent_servers": {
    "JonsOC": {
      "command": "/absolute/path/bin/jonsoc",
      "args": ["acp"]
    }
  }
}
...
To open it, use the new 'JonsOC' agent in the AI Chat agent selector.
...
To use JonsOC as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use JonsOC as the ACP agent for chat.
...
JonsOC works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your JonsOC config

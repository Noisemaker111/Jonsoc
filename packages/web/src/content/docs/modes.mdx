...
:::caution
Modes are now configured through the `agent` option in the jonsoc config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in jonsoc allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the jonsoc config.
...
jonsoc comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.jonsoc/plans/*.md` to detail the plan itself
...
Configure modes in your `jonsoc.json` config file:
...
- Global: `~/.config/jonsoc/modes/`
- Project: `.jonsoc/modes/`

```markdown title="~/.config/jonsoc/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global jonsoc config and the project specific config.
...
Create mode files in `.jonsoc/modes/` for project-specific modes or `~/.config/jonsoc/modes/` for global modes:

```markdown title=".jonsoc/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/jonsoc/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:

...
There are two types of agents in JonsOC; primary agents and subagents.
...
JonsOC comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
JonsOC comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

JonsOC comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `jonsoc.json` config file:
...
- Global: `~/.config/jonsoc/agents/`
- Per-project: `.jonsoc/agents/`

```markdown title="~/.config/jonsoc/agents/review.md"
---
...
If no temperature is specified, JonsOC uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global JonsOC config and the project specific config.
...
The model ID in your JonsOC config uses the format `provider/model-id`. For example, if you're using [JonsOC Zen](/docs/zen), you would use `jonsoc/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="jonsoc.json" {3-6,9-12}
{
  "$schema": "https://jonsoc.ai/config.json",
  "tools": {
    "write": true,
...
```json title="jonsoc.json" {3-5,8-10}
{
  "$schema": "https://jonsoc.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `jonsoc models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/jonsoc).
:::
...
```markdown title="~/.config/jonsoc/agents/docs-writer.md"
---
...
```markdown title="~/.config/jonsoc/agents/security-auditor.md"
---

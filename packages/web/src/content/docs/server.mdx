---
title: Server
description: Interact with jonsoc server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `jonsoc serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an jonsoc client can use.
...
```bash
jonsoc serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
jonsoc serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `jonsoc`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `jonsoc serve` and `jonsoc web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password jonsoc serve
```
...
When you run `jonsoc` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the jonsoc server to interact with jonsoc programmatically.
:::

This architecture lets jonsoc support multiple clients and allows you to interact with jonsoc programmatically.

You can run `jonsoc serve` to start a standalone server. If you have the
jonsoc TUI running, `jonsoc serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the JonsOC [IDE](/docs/ide) plugins.
...
The jonsoc server exposes the following APIs.

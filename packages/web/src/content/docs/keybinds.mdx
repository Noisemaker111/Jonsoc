...
JonsOC has a list of keybinds that you can customize through the JonsOC config.
...
JonsOC uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The JonsOC desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `jonsoc.json`.

...
You can add external tools to JonsOC using the _Model Context Protocol_, or MCP. JonsOC supports both local and remote servers.
...
You can define MCP servers in your [JonsOC Config](https://jonsoc.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="jonsoc.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/jonsoc` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
JonsOC automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, JonsOC will:
...
If the server requires authentication, JonsOC will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `jonsoc mcp auth <server-name>`.
...
```bash
jonsoc mcp auth my-oauth-server
```
...
```bash
jonsoc mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, JonsOC will store the tokens securely in `~/.local/share/jonsoc/mcp-auth.json`.
...
jonsoc mcp debug my-oauth-server
```
...
Your MCPs are available as tools in JonsOC, alongside built-in tools. So you can manage them through the JonsOC config like any other tool.
...
```json title="jonsoc.json" {11, 14-18}
{
  "$schema": "https://jonsoc.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
jonsoc mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect JonsOC to your Sentry account.

...
With JonsOC you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, JonsOC uses our own `jonsoc` theme.
...
JonsOC comes with several built-in themes.
...
- Want JonsOC to match their terminal's appearance
...
JonsOC supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/jonsoc/themes/*.json` or `$XDG_CONFIG_HOME/jonsoc/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.jonsoc/themes/*.json`
4. **Current working directory** - Defined in `./.jonsoc/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/jonsoc/themes
vim ~/.config/jonsoc/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .jonsoc/themes
vim .jonsoc/themes/my-theme.json
```
...
{
  "$schema": "https://jonsoc.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",

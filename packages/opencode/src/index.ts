...
async function shouldRunLocalDev(cwd: string) {
  ...
  const isRoot = name === "jonsoc" || name === "opencode"
  if (!isRoot) return false
  const entry = Bun.file(path.join(cwd, "packages", "opencode", "src", "index.ts"))
  return entry.exists()
}
...
async function runCli() {
  ...
  const { NamedError } = await import("@opencode-ai/util/error")

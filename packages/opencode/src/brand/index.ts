export namespace Brand {
  ...
  export const DOMAIN = process.env.JONSOC_DOMAIN ?? process.env.OPENCODE_DOMAIN ?? `${BRAND_LOWER}.ai`
  ...
  export const LEGACY_WELL_KNOWN_PATH = "/.well-known/opencode"
  ...
  export const CONFIG_FILES: string[] = (() => {
    ...
    if (ALLOW_LEGACY_OPENCODE_CONFIGS) {
      files.push("opencode.jsonc", "opencode.json")
    }
    return files
  })()
  ...
  export const LEGACY_CLI_NAME = "opencode"

  export const HOMEBREW_TAP = process.env.JONSOC_HOMEBREW_TAP ?? process.env.OPENCODE_HOMEBREW_TAP ?? "sst/homebrew-tap"

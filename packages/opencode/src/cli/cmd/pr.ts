...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run jonsoc",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for jonsoc session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found jonsoc session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`jonsoc import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting jonsoc...")
        UI.println()

        // Launch jonsoc TUI with session ID if available
        const { spawn } = await import("child_process")
        const argv = sessionId ? ["-s", sessionId] : []
        const proc = spawn("jonsoc", argv, {
          stdio: "inherit",
          cwd: process.cwd(),
        })
        ...
        await new Promise<void>((resolve, reject) => {
          proc.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`jonsoc exited with code ${code}`))
          })

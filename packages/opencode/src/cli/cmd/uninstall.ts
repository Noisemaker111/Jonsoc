...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall jonsoc and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall JonsOC")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g jonsoc-ai",
      pnpm: "pnpm uninstall -g jonsoc-ai",
      bun: "bun remove -g jonsoc-ai",
      yarn: "yarn global remove jonsoc-ai",
      brew: "brew uninstall jonsoc",
      choco: "choco uninstall jonsoc",
      scoop: "scoop uninstall jonsoc",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall jonsoc -y -r`.quiet().nothrow()
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".jonsoc") || binDir.includes(".opencode")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using JonsOC!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      ...
      .catch(() => "")
    if (
      content.includes("# jonsoc") ||
      content.includes(".jonsoc/bin") ||
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (trimmed === "# jonsoc" || trimmed === "# opencode") {
      skip = true
      continue
    }
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".jonsoc/bin") || trimmed.includes(".opencode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && (trimmed.includes(".jonsoc/bin") || trimmed.includes(".opencode/bin"))) ||
      (trimmed.startsWith("fish_add_path") && (trimmed.includes(".jonsoc") || trimmed.includes(".opencode")))
    ) {
      continue
    }

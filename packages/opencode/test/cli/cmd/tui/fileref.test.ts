...
describe("File reference regex", () => {
  test("matches file with line number", () => {
    ...
    const text = "packages/opencode/src/cli/ui.ts:82"
    const matches = [...text.matchAll(regex)]
    expect(matches.length).toBeGreaterThan(0)
    expect(matches[0][2]).toBe("packages/opencode/src/cli/ui.ts")
  ...
  test("matches file in text", () => {
    const regex = /([`'"<]?)([\w\-./]+\/[\w\-./]+(?:\.[\w]+)?)(?::(\d+))?/g
    const text = "Check packages/opencode/src/cli/ui.ts:82 for details"
    const matches = [...text.matchAll(regex)]

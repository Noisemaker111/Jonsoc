...
description: What's new in JonsOC 1.0.
---

JonsOC 1.0 is a complete rewrite of the TUI.
...
The new TUI works like the old one since it connects to the same jonsoc server.
...
version. However some older versions of JonsOC always grab latest.

To upgrade manually, run

```bash
$ jonsoc upgrade 1.0.0
```

To downgrade back to 0.x, run

```bash
$ jonsoc upgrade 0.15.31
```

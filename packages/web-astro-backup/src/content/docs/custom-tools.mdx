---
title: Custom Tools
description: Create tools the LLM can call in jonsoc.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside jonsoc's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.jonsoc/tools/` directory of your project.
- Or globally, by placing them in `~/.config/jonsoc/tools/`.
...
```ts title=".jonsoc/tools/database.ts" {1}
...
```ts title=".jonsoc/tools/project.ts" {8}
import { tool } from "@jonsoc/plugin"
...
```python title=".jonsoc/tools/add.py"
import sys
...
```ts title=".jonsoc/tools/python-add.ts" {10}
import { tool } from "@jonsoc/plugin"
...
export default tool({
  ...
  async execute(args) {
    const result = await Bun.$`python3 .jonsoc/tools/add.py ${args.a} ${args.b}`.text()
    return result.trim()
  },
})
```

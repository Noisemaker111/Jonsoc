...
To debug any issues with JonsOC, you can check the logs or the session data
...
- **macOS/Linux**: `~/.local/share/jonsoc/log/`
- **Windows**: `%USERPROFILE%\.local\share\jonsoc\log\`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `jonsoc --log-level DEBUG`.

---

### Storage

jonsoc stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/jonsoc/`
- **Windows**: `%USERPROFILE%\.local\share\jonsoc`
...
If you're experiencing issues with JonsOC:

1. **Report issues on GitHub**

   The best way to report bugs or request features is through our GitHub repository:

   [**github.com/anomalyco/jonsoc/issues**](https://github.com/anomalyco/jonsoc/issues)
...
2. **Join our Discord**
   ...
   [**jonsoc.com/discord**](https://jonsoc.com/discord)
...
3. Ensure you have the latest version with `jonsoc upgrade`
...
To figure out what models you have access to, run `jonsoc models`
...
If you encounter API call errors, this may be due to outdated provider packages. jonsoc dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
2. Restart jonsoc to reinstall the latest provider packages

This will force jonsoc to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
jonsoc will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.

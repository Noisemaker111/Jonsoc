...
export async function runUpdater({ alertOnFail }: { alertOnFail: boolean }) {
  ...
  if (!update) {
    if (alertOnFail)
      await message("You are already using the latest version of JonsOC", { title: "No Update Available" })
    return
  }
  ...
  const shouldUpdate = await ask(
    `Version ${update.version} of JonsOC has been downloaded, would you like to install it and relaunch?`,

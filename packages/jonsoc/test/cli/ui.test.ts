...
describe("UI.markdown", () => {
  test("converts file references with context keyword to OSC 8 hyperlinks", () => {
    const text = "See packages/jonsoc/src/cli/ui.ts:81 for details"
    const result = UI.markdown(text)

    expect(result).toContain("\x1b]8;;")
    expect(result).toContain("file://")
    expect(result).toContain("packages/jonsoc/src/cli/ui.ts:81")
  })

  test("handles file references without line numbers", () => {
    const text = "Check packages/jonsoc/src/cli/ui.ts"
    const result = UI.markdown(text)
  ...
  test("handles quoted file references", () => {
    const text = "Look at 'packages/jonsoc/src/cli/ui.ts:81'"
    const result = UI.markdown(text)
  ...
  test("handles Windows paths with backslashes", () => {
    const text = "Edit packages\\jonsoc\\src\\cli\\ui.ts:81"
    const result = UI.markdown(text)

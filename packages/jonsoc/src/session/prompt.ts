    ...
    while (true) {
      ...
      if (step === 1) {
        ensureTitle({
          ...
          log.error("Failed to generate session title", { error: err, sessionID: session.id })
        })
      }
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
  ...
  async function ensureTitle(input: {
    ...
    if (!agent) {
      throw new Error("Title agent not found - check your agent configuration")
    }
    ...
    return Session.update(
      ...
      (draft) => {
        ...
        if (!cleaned) {
          throw new Error("Title generation produced no valid text after cleaning")
        }

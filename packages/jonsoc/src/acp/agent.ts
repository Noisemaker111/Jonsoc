...
import type { Event, OpencodeClient, SessionMessageResponse } from "@jonsoc/sdk/v2"
...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `jonsoc auth login` in the terminal",
        name: "Login with jonsoc",
        id: "jonsoc-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // JonsOC stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const jonsocProvider = providers.find((p) => p.id === "jonsoc")
    if (jonsocProvider) {
      if (jonsocProvider.models["big-pickle"]) {
        return { providerID: "jonsoc", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(jonsocProvider.models))
      if (best) {
        return {

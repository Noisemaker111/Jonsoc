...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for jonsoc.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal jonsoc sessions
...
jonsoc acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via jonsoc's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | jonsoc acp
```
...
ACP sessions map cleanly to jonsoc's internal session model:

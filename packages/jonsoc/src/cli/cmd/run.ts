...
import { createOpencodeClient, type OpencodeClient } from "@jonsoc/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run jonsoc with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running jonsoc server (e.g., http://localhost:4096)",
      })
      .option("port", {
        type: "number",
  ...
  handler: async (args) => {
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createOpencodeClient({ baseUrl: "http://jonsoc.internal", fetch: fetchFn })

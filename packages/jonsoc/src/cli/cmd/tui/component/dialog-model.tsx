...
export function useConnected() {
  const sync = useSync()
  return createMemo(() =>
    sync.data.provider.some((x) => x.id !== "jonsoc" || Object.values(x.models).some((y) => y.cost?.input !== 0)),
  )
}
          ...
          return [
            {
              ...
              category: "Favorites",
              disabled: provider.id === "jonsoc" && model.id.includes("-nano"),
              footer: model.cost?.input === 0 && provider.id === "jonsoc" ? "Free" : undefined,
              onSelect: () => {
                dialog.clear()
                local.model.set(
                  {
        ...
        pipe(
          ...
          map(([model, info]) => {
            ...
            return {
              ...
              disabled: provider.id === "jonsoc" && model.includes("-nano"),
              footer: info.cost?.input === 0 && provider.id === "jonsoc" ? "Free" : undefined,
              onSelect() {
                dialog.clear()
                local.model.set(
                  {

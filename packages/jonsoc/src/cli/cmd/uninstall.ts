...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g jonsoc",
      pnpm: "pnpm uninstall -g jonsoc",
      bun: "bun remove -g jonsoc",
      yarn: "yarn global remove jonsoc",
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".jonsoc") || binDir.includes(".jonsoc")) {
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (trimmed === "# jonsoc" || trimmed === "# jonsoc") {
      skip = true
      continue
    }
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".jonsoc/bin") || trimmed.includes(".jonsoc/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && (trimmed.includes(".jonsoc/bin") || trimmed.includes(".jonsoc/bin"))) ||
      (trimmed.startsWith("fish_add_path") && (trimmed.includes(".jonsoc") || trimmed.includes(".jonsoc")))
    ) {
      continue
    }

...
## Decision: How We Should Add i18n To `@jonsoc/ui`
...
- Export them from `@jonsoc/ui` via `packages/ui/package.json` exports (e.g. `"./i18n/*": "./src/i18n/*.ts"`).
...
- `packages/app/`:
  ...
  - Add a tiny bridge provider in `packages/app/src/app.tsx` to feed `useLanguage()` into `@jonsoc/ui`'s `I18nProvider`.
...
- `packages/enterprise/`:
  ...
  - Merge `@jonsoc/ui` dictionaries and (optionally) enterprise-local dictionaries.
...
1. Completed (2026-01-20): Add `@jonsoc/ui` i18n context (`packages/ui/src/context/i18n.tsx`) + export it.

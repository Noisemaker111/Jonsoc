...
import { createOpencodeClient } from "@jonsoc/sdk"
...
function createOpencode() {
  ...
  const proc = spawn(`jonsoc`, [`serve`, `--hostname=${host}`, `--port=${port}`])
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/jonsoc|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/jonsoc` or `/oc`")
  }
}
...
async function assertOpencodeConnected() {
  ...
  if (!connected) {
    throw new Error("Failed to connect to jonsoc server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.jonsoc.com" : "https://jonsoc.com"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.jonsoc.com/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("jonsoc-github-action")
    response = await fetch("https://api.jonsoc.com/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/jonsoc" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/jonsoc") || body.includes("/oc")) {
      if (reviewContext) {
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "jonsoc-agent[bot]"`
  await $`git config --global user.email "jonsoc-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `jonsoc/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/jonsoc-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[jonsoc session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""

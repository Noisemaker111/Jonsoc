...
brew install jonsoc              # macOS 和 Linux（官方 brew formula，更新频率较低）
paru -S jonsoc-bin               # Arch Linux
mise use -g jonsoc               # 任意系统
nix run nixpkgs#jonsoc           # 或用 github:anomalyco/jonsoc 获取最新 dev 分支
```
...
JonsOC 也提供桌面版应用。可直接从 [发布页 (releases page)](https://github.com/anomalyco/jonsoc/releases) 或 [jonsoc.com/download](https://jonsoc.com/download) 下载。
...
JonsOC 内置两种 Agent，可用 `Tab` 键快速切换：
...
了解更多 [Agents](https://jonsoc.com/docs/agents) 相关信息。

### 文档

更多配置说明请查看我们的 [**官方文档**](https://jonsoc.com/docs)。
...
如果你在项目名中使用了 “jonsoc”（如 “jonsoc-dashboard” 或 “jonsoc-mobile”），请在 README 里注明该项目不是 JonsOC 团队官方开发，且不存在隶属关系。
...
- 100% 开源。
- 不绑定特定提供商。推荐使用 [JonsOC Zen](https://jonsoc.com/zen) 的模型，但也可搭配 Claude、OpenAI、Google 甚至本地模型。模型迭代会缩小差异、降低成本，因此保持 provider-agnostic 很重要。
- 内置 LSP 支持。
- 聚焦终端界面 (TUI)。JonsOC 由 Neovim 爱好者和 [terminal.shop](https://terminal.shop) 的创建者打造，会持续探索终端的极限。
...
**加入我们的社区** [Discord](https://discord.gg/jonsoc) | [X.com](https://x.com/jonsoc)

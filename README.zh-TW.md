...
brew install anomalyco/tap/jonsoc # macOS 與 Linux（推薦，始終保持最新）
brew install jonsoc              # macOS 與 Linux（官方 brew formula，更新頻率較低）
paru -S jonsoc-bin               # Arch Linux
mise use -g github:anomalyco/jonsoc    # 任何作業系統
nix run nixpkgs#jonsoc           # 或使用 github:anomalyco/jonsoc 以取得最新開發分支
```
...
JonsOC 內建了兩種 Agent，您可以使用 `Tab` 鍵快速切換。
...
如果您有興趣參與 JonsOC 的開發，請在提交 Pull Request 前先閱讀我們的 [貢獻指南 (Contributing Docs)](./CONTRIBUTING.md)。

### 基於 JonsOC 進行開發

如果您正在開發與 JonsOC 相關的專案，並在名稱中使用了 "jonsoc"（例如 "jonsoc-dashboard" 或 "jonsoc-mobile"），請在您的 README 中加入聲明，說明該專案並非由 JonsOC 團隊開發，且與我們沒有任何隸屬關係。
...
**加入我們的社群** [Discord](https://discord.gg/jonsoc) | [X.com](https://x.com/jonsoc)
